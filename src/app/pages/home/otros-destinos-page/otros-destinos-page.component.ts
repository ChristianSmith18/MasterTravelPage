...
@Component({
  selector: 'app-otros-destinos-page',
  templateUrl: './otros-destinos-page.component.html',
  styleUrls: ['./otros-destinos-page.component.scss']
})
export class OtrosDestinosPageComponent implements AfterContentChecked {
  ...
  constructor(
    ...
    this.title.setTitle('Otros Destinos');
    ...
    this.database.getAllData().subscribe(
      (data: dataInfo) => {
        if (key) {
          this.database.setVisitasOtrosCount(data.visitasOtros + 1);
          key = false;
        }
      }
    );
